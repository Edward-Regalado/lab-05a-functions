...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  ...
  let thirdSum = `${a} and ${b} and ${c} sum to ${firstSum}.`;
  let fourthSum = `The product of ${a} and ${b} and ${c} is ${secondSum}.`;
  //console.log([firstSum, secondSum, thirdSum, fourthSum]);
...
function sumArray(sumArr) { //eslint-disable-line
  ...
  var numTwo = sum(numOne, sumArr[2])[0];
...
function multiplyArray(multArr) { //eslint-disable-line
  var numOne = multiply(multArr[0],multArr[1])[0];
  var numTwo = multiply(numOne,multArr[2])[0];

  var str = `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${numTwo}.`;
  // console.log([numTwo, str]);
  return [numTwo, str];
}
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
  let prod = dynamicArray[0];
  let messArray =[];

  for (var i = 0; i < dynamicArray.length; i ++) {
    if (i < (dynamicArray.length-1)) {
      messArray += dynamicArray[i] + ',';
    } else {
      messArray += dynamicArray[i];
    }

    if (i < (dynamicArray.length-1)) {
      prod = multiply(prod, dynamicArray[i + 1])[0];
    }

  }

  let message = `The numbers ${messArray} have a product of ${prod}.`;
  //console.log(message);
  //console.log(prod);
